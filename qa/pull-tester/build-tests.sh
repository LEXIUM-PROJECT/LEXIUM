...
if [ -d "$OUT_DIR" -a -w "$OUT_DIR" ]; then
  set +e
  # Windows:
  cp /e/development/LEXIUM/chainfork/chaincoin/win32-build/src/bitcoind.exe $OUT_DIR/bitcoind.exe
  cp /e/development/LEXIUM/chainfork/chaincoin/win32-build/src/test/test_lexium.exe $OUT_DIR/test_lexium.exe
  cp /e/development/LEXIUM/chainfork/chaincoin/win32-build/src/qt/bitcoind-qt.exe $OUT_DIR/bitcoin-qt.exe
  # Linux:
  cp /e/development/LEXIUM/chainfork/chaincoin/linux-build/src/bitcoind $OUT_DIR/bitcoind
  cp /e/development/LEXIUM/chainfork/chaincoin/linux-build/src/test/test_lexium $OUT_DIR/test_lexium
  cp /e/development/LEXIUM/chainfork/chaincoin/linux-build/src/qt/bitcoind-qt $OUT_DIR/bitcoin-qt
  set -e
fi

# Run unit tests and blockchain-tester on Linux:
cd /e/development/LEXIUM/chainfork/chaincoin/linux-build
make check

# Run RPC integration test on Linux:
/e/development/LEXIUM/chainfork/chaincoin/qa/rpc-tests/wallet.sh /e/development/LEXIUM/chainfork/chaincoin/linux-build/src
/e/development/LEXIUM/chainfork/chaincoin/qa/rpc-tests/listtransactions.py --srcdir /e/development/LEXIUM/chainfork/chaincoin/linux-build/src
# Clean up cache/ directory that the python regression tests create
rm -rf cache

if [ $RUN_EXPENSIVE_TESTS = 1 ]; then
  # Run unit tests and blockchain-tester on Windows:
  cd /e/development/LEXIUM/chainfork/chaincoin/win32-build
  make check
fi
