        ...
        for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it)
        {
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++) {
                ...
                if(coin_type == ONLY_DENOMINATED) {
                    ...
                    found = !IsDenominatedAmount(pcoin->vout[i].nValue);
                    if(found && coin_type == ONLY_NONDENOMINATED_NOTMN) found = (pcoin->vout[i].nValue != 25000*COIN || pcoin->vout[i].nValue != 0.1234*COIN); // do not use MN funds
                } else {
                    found = true;
                }
                if(!found) continue;
...
bool CWallet::SelectCoinsDark(int64_t nValueMin, int64_t nValueMax, std::vector<CTxIn>& setCoinsRet, int64_t& nValueRet, int nDarksendRoundsMin, int nDarksendRoundsMax) const
{
    ...
    BOOST_FOREACH(const COutput& out, vCoins)
    {
        ...
        if(fMasterNode && out.tx->vout[out.i].nValue == 25000*COIN || out.tx->vout[out.i].nValue == 0.1234*COIN) continue; //masternode input
