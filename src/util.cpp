...
//int64_t enforceMasternodePaymentsTime = 4085657524; // Lexium
...
void ParseParameters(int argc, const char* const argv[])
{
    ...
    BOOST_FOREACH(const PAIRTYPE(string,string)& entry, mapArgs)
    {
        ...
        if (name.find("--") == 0)
        {
            std::string singleLexium(name.begin()+1, name.end());
            if (mapArgs.count(singleLexium) == 0)
                mapArgs[singleLexium] = entry.second;
            name = singleLexium;
        }
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Lexium
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Lexium
    // Mac: ~/Library/Application Support/Lexium
    // Unix: ~/.lexium
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Lexium";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "lexium.conf"));
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty lexium.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override lexium.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "lexiumd.pid"));
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong Lexium will not work properly.");
