    ...
    if(status == MASTERNODE_NOT_PROCESSED) {
        ...
        if (chainActive.Height() >= Params().MasternodePortForkHeight())
        {
            if(Params().NetworkID() == CChainParams::MAIN){
                if(service.GetPort() != 11884) {
                    notCapableReason = "Invalid port: " + boost::lexical_cast<string>(service.GetPort()) + " - only 11884 is supported on mainnet.";
                    status = MASTERNODE_NOT_CAPABLE;
                    LogPrintf("CActiveMasternode::ManageStatus() - not capable: %s\n", notCapableReason.c_str());
                    return;
                }
            } else if(service.GetPort() == 11884) {
                notCapableReason = "Invalid port: " + boost::lexical_cast<string>(service.GetPort()) + " - 11884 is only supported on mainnet.";
                status = MASTERNODE_NOT_CAPABLE;
                LogPrintf("CActiveMasternode::ManageStatus() - not capable: %s\n", notCapableReason.c_str());
                return;
            }
